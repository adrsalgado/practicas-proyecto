<?php
    ...
    $loteProd=consultasSQL::clean_string($_POST['prod-lote']);
    $proveedorProd=consultasSQL::clean_string($_POST['prod-proveedor']);
    $cateProd=consultasSQL::clean_string($_POST['prod-categoria']);
    $modelProd=consultasSQL::clean_string($_POST['prod-model']);
    $presentacionProd=consultasSQL::clean_string($_POST['prod-presentacion']);
    ...
    if($codeProd!="" && $nameProd!="" && $loteProd!="" && $proveedorProd!="" && $cateProd!="" && $presentacionProd!="" && $modelProd!="" && $marcaProd!="" && $stockProd!="" && $codePProd!=""){
            ...
            if($imgType=="image/jpeg" || $imgType=="image/png"){
                if(($imgSize/1024)<=$imgMaxSize){
                    ...
                    if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/img-products/".$imgFinalName)){
                        if(consultasSQL::InsertSQL("producto", "CodigoProd, lote, NombreProd, CodigoCat, proveedor, marca, presentacion, vencimiento, invima, Marca, Stock, NITProveedor, Imagen, Nombre, Estado", "'$codeProd','$nameProd','$cateProd','$loteProd', '$proveedorProd','$marcaProd','$presentacionProd','$descProd', '$modelProd','$marcaProd','$stockProd','$codePProd','$imgFinalName','$adminProd', '$estadoProd'")){
                            echo '<script>
                                swal({
