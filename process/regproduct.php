<?php
    ...
    if($codeProd!="" && $nameProd!="" && $cateProd!="" && $loteProd!="" && $presentacionProd!="" && $modelProd!="" && $marcaProd!="" && $stockProd!="" && $codePProd!=""){
            ...
            if($imgType=="image/jpeg" || $imgType=="image/png"){
                if(($imgSize/1024)<=$imgMaxSize){
                    ...
                    if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/img-products/".$imgFinalName)){
                        if(consultasSQL::InsertSQL("producto", "CodigoProd, NombreProd, CodigoCat, lote, presentacion, vencimiento, invima, Marca, Stock, NITProveedor, Imagen, Nombre, Estado", "'$codeProd','$nameProd','$cateProd','$loteProd', '$presentacionProd','$descProd', '$modelProd','$marcaProd','$stockProd','$codePProd','$imgFinalName','$adminProd', '$estadoProd'")){
                            echo '<script>
                                swal({
